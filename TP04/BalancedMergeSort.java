...
public class BalancedMergeSort {
    ...
    public void deleteAllFiles() {
        for (int i = 0; i < paths * 2; i++) {
            File file = new File("output/bms/tmp" + i + SUFFIX);
            if (file.exists()) {
                if (file.delete()) {
    ...
    public static int fragment(int ram, int paths, String key) {
        ...
        try {
            ...
            for (int i = 0; i < paths; i++) {
                temp[i] = new RandomAccessFile("output/bms/tmp" + i + SUFFIX, "rw"); // Create temp files
            }
    ...
    public static void merge(int ram, int paths, boolean isBase, String key) {
        ...
        for (int i = 0; i < paths; i++) {
            temp1[i] = new HeroFile("output/bms/tmp" + (isBase ? i : i + paths) + SUFFIX);
            temp2[i] = new HeroFile("output/bms/tmp" + (isBase ? i + paths : i) + SUFFIX);
        }
        ...
        try {
            while (true) {
                ...
                List<Map.Entry<HeroFile, Hero>> ordered = map.entrySet().stream().sorted(Map.Entry.comparingByValue(HeroComparator.byId())).collect(Collectors.toList());
